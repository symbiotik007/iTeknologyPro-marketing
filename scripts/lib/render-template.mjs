...
import { icons, iconKeys } from "../template/icons.mjs";
...
export async function renderFeatureHighlight(opts, outPath) {
  const { headline, subtitle, cta, icon } = opts;
  const iconKey = icon && icons[icon] ? icon : iconKeys[0];

  let html = await readFile(path.join(TEMPLATE_DIR, "feature-highlight.html"), "utf8");
  html = html
...
// Slide 1: hook/cover. opts: { badge, hook: {text, accent} }
export function renderHook(opts, outPath) {
  return renderSlide("slide-1-hook.html", {
    BADGE_TEXT: escapeHtml(opts.badge),
    HOOK_HTML: headlineHtml(opts.hook),
  }, outPath);
}

// Slide 2: problema. opts: { title, items: string[] } (cada item = 1 dolor)
const WARN_ICON = `<svg viewBox="0 0 24 24" fill="none"><path d="M12 3l10 18H2L12 3z" stroke="#FF6B6B" stroke-width="2" stroke-linejoin="round"/><path d="M12 10v5M12 18h.01" stroke="#FF6B6B" stroke-width="2" stroke-linecap="round"/></svg>`;
export function renderProblem(opts, outPath) {
  const itemsHtml = opts.items
    .map((it) => `<div class="item">${WARN_ICON}<span>${escapeHtml(it)}</span></div>`)
    .join("\n");
  return renderSlide("slide-2-problem.html", {
    TITLE_HTML: headlineHtml(opts.title),
    ITEMS_HTML: itemsHtml,
  }, outPath);
}

// Slide 3: solución. opts: { title, items: string[] } (cada item = 1 feature)
...
export function renderSolution(opts, outPath) {
  const itemsHtml = opts.items
    ...
    .join("\n");
  return renderSlide("slide-3-solution.html", {
    TITLE_HTML: headlineHtml(opts.title),
    ITEMS_HTML: itemsHtml,
  }, outPath);
}

// Slide 4: prueba social. opts: { quote, statNumber, statLabel, source }
export function renderSocialProof(opts, outPath) {
  return renderSlide("slide-4-social-proof.html", {
...
// Slide 5: CTA final. opts: { ctaHeadline, ctaButton, handle }
