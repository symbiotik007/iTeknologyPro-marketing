# Vault de prompts de imágenes

Banco de prompts para generar imágenes de posts. Reglas:

- Formato: cuadrado **1080x1080** (o 2048x2048 y el script lo baja). JPEG/PNG.
- Marca: azul **#2563eb** + blanco. Estilo minimalista, moderno, cálido.
- CTA visible: botón "Crea tu tienda gratis". Link real de los posts: **https://t.co/phJghRDEoU**
- Al usar un prompt, añade el tag **`-USED`** al lado del título y anota el archivo (`→ 0N.jpg`).
- Los `-USED` ya salieron; para el próximo lote toma los que NO tienen `-USED`.

Leyenda: `-USED` = ya publicado · (sin tag) = disponible.

---

## Usados

### Tu tienda en minutos -USED → 01.jpg
```
Post cuadrado 1080x1080 para Instagram y Facebook de una marca de tecnología llamada "iTeknology".
Paleta: azul #2563eb y blanco. Estilo minimalista, moderno, limpio, con formas geométricas suaves y un mockup sutil de una tienda online en un smartphone.
Titular grande y legible en el centro: "TU TIENDA ONLINE EN MINUTOS".
Debajo, texto pequeño: "Sin código. Empieza gratis."
Deja espacio limpio en la parte inferior para el logo y un botón azul que diga "Crea tu tienda gratis".
Alta calidad, sin marcas de agua, tipografía sans-serif nítida.
```

### Vende 24/7 -USED → 02.jpg
```
Post cuadrado 1080x1080 para redes sociales, marca "iTeknology". Paleta azul #2563eb y blanco.
Estilo minimalista y profesional, con un ícono grande de reloj o carrito de compras y elementos tech ligeros.
Titular destacado: "VENDE 24/7 SIN COMPLICARTE".
Texto de apoyo pequeño: "Tus clientes compran a cualquier hora."
Zona inferior libre para logo y botón azul "Crea tu tienda gratis".
Alta calidad, sin marcas de agua, tipografía sans-serif grande y legible en móvil.
```

### De WhatsApp a tienda profesional -USED → 03.jpg
```
Post cuadrado 1080x1080 para Instagram, marca de tecnología "iTeknology". Colores azul #2563eb y blanco.
Concepto antes/después: a la izquierda un chat de WhatsApp simple, a la derecha una tienda online profesional en un teléfono. Flecha entre ambos.
Titular: "DE WHATSAPP A TIENDA PROFESIONAL".
Estilo limpio, minimalista, moderno. Espacio inferior para logo y botón azul "Crea tu tienda gratis".
Alta calidad, sin marcas de agua, tipografía nítida.
```

### Restaurante, tienda o delivery -USED → 04.jpg
```
Post cuadrado 1080x1080 para redes, marca "iTeknology". Paleta azul #2563eb y blanco.
Muestra 3 íconos claros en fila: una hamburguesa (restaurante), una bolsa de compras (tienda) y una moto de delivery.
Titular arriba: "RESTAURANTE, TIENDA O DELIVERY".
Subtítulo: "Todo en una sola plataforma."
Estilo minimalista, geométrico, moderno. Zona inferior para logo y botón azul "Crea tu tienda gratis".
Alta calidad, sin marcas de agua, tipografía sans-serif legible.
```

### Empieza gratis hoy -USED → 05.jpg
```
Post cuadrado 1080x1080 motivacional para Instagram y Facebook, marca de tecnología "iTeknology".
Fondo azul #2563eb con detalles geométricos blancos, o blanco con acentos azules. Look premium y limpio.
Titular grande y centrado: "EMPIEZA GRATIS HOY".
Subtítulo pequeño: "El mejor momento para tu negocio es ahora."
Deja espacio para logo y un botón blanco o azul que diga "Crea tu tienda gratis".
Alta calidad, sin marcas de agua, tipografía sans-serif audaz y legible.
```

---

## Disponibles (próximo lote)

### Tu marca, tus reglas
```
Post cuadrado 1080x1080 para redes, marca de tecnología "iTeknology". Azul #2563eb y blanco.
Estilo minimalista con un mockup de tienda online personalizada (logo y colores propios) en un smartphone.
Titular: "TU MARCA. TU TIENDA. TUS REGLAS."
Subtítulo: "Control total de tu negocio."
Espacio inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### Sin código
```
Post cuadrado 1080x1080, marca "iTeknology". Paleta azul #2563eb y blanco.
Concepto: una persona sonriente creando su tienda en una laptop, con íconos flotantes de arrastrar-y-soltar. Estilo ilustración plana moderna y cálida.
Titular: "SIN CÓDIGO. SIN LÍMITES."
Subtítulo: "Si sabes usar WhatsApp, sabes crear tu tienda."
Zona inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### Cientos de negocios
```
Post cuadrado 1080x1080, marca "iTeknology". Azul #2563eb y blanco.
Estilo moderno con una cuadrícula de mini-tiendas/logos variados y un check de confianza.
Titular: "CIENTOS DE NEGOCIOS YA VENDEN AQUÍ."
Subtítulo: "Únete hoy."
Espacio inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### Gestiona todo en un panel
```
Post cuadrado 1080x1080, marca de tecnología "iTeknology". Paleta azul #2563eb y blanco.
Muestra un dashboard limpio en un portátil con pedidos, repartidores en un mapa y gráficas de ventas.
Titular: "TU NEGOCIO COMPLETO EN UN PANEL."
Subtítulo: "Pedidos, repartidores y ventas, en un solo lugar."
Zona inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### Cobra en línea
```
Post cuadrado 1080x1080, marca "iTeknology". Azul #2563eb y blanco.
Concepto: un teléfono mostrando una compra exitosa con métodos de pago (tarjeta, Nequi, transferencia) e íconos de seguridad.
Titular: "COBRA EN LÍNEA, CRECE SIN LÍMITES."
Subtítulo: "Pagos fáciles y seguros."
Espacio inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### El mejor momento es hoy
```
Post cuadrado 1080x1080 motivacional, marca "iTeknology". Fondo azul #2563eb con detalles blancos.
Concepto: un reloj o un cohete despegando, sensación de impulso y oportunidad.
Titular: "EL MEJOR MOMENTO ES HOY."
Subtítulo: "Tu tienda online, lista en minutos."
Espacio inferior para logo y botón blanco "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### Domicilios con tus repartidores
```
Post cuadrado 1080x1080, marca "iTeknology". Azul #2563eb y blanco.
Muestra una moto de delivery con GPS en un mapa y un paquete. Estilo ilustración plana moderna.
Titular: "TUS DOMICILIOS, BAJO CONTROL."
Subtítulo: "Repartidores y seguimiento en vivo."
Zona inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### De la idea a la venta
```
Post cuadrado 1080x1080, marca "iTeknology". Paleta azul #2563eb y blanco.
Concepto: una bombilla de idea que se transforma en una tienda online y luego en una notificación de venta. Secuencia visual de 3 pasos.
Titular: "DE LA IDEA A TU PRIMERA VENTA."
Subtítulo: "En un solo día."
Espacio inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### No más catálogo en el chat
```
Post cuadrado 1080x1080, marca "iTeknology". Azul #2563eb y blanco.
Concepto antes/después: un chat lleno de fotos desordenadas vs. una tienda online ordenada y profesional.
Titular: "DEJA EL CATÁLOGO EN EL CHAT."
Subtítulo: "Dale a tus clientes una tienda de verdad."
Zona inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```

### Tu tienda no tiene que ser cara
```
Post cuadrado 1080x1080, marca "iTeknology". Fondo blanco con acentos azul #2563eb.
Estilo limpio y premium con un smartphone mostrando una tienda elegante.
Titular: "PROFESIONAL NO SIGNIFICA CARO."
Subtítulo: "Tu tienda online, rápida y lista para vender."
Espacio inferior para logo y botón azul "Crea tu tienda gratis". Alta calidad, sin marcas de agua.
```
